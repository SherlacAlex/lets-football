    ...
    <div>
      <div class="flex justify-between items-center mb-6">
        <div>
          ...
          <p class="text-slate-400 text-xs mt-1">Enter your score predictions below. Exact score = 3pts, Correct outcome = 1pt.</p>
        </div>
        <div class="flex items-center space-x-2 bg-slate-900 border border-slate-800 rounded-xl px-3 py-1.5 text-xs text-slate-400">
      ...
      <!-- Action Toast Banner -->
      <div v-if="toastMessage" :class="[toastType === 'success' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-rose-500/10 border-rose-500/20 text-rose-400', 'p-4 mb-6 rounded-2xl border text-sm flex items-center justify-between transition-all duration-300']">
        <div class="flex items-center space-x-2">
          <UIcon :name="toastType === 'success' ? 'i-heroicons-check-circle' : 'i-heroicons-exclamation-triangle'" class="w-5 h-5" />
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <!-- Match Cards -->
        <div v-for="match in matches" :key="match.id" class="group bg-slate-900/40 border border-slate-800 hover:border-slate-700/80 rounded-3xl p-6 backdrop-blur-md transition-all duration-300">
          <div class="flex justify-between items-center text-xs text-slate-500 mb-4">
            <span>{{ match.date }} &bull; Group {{ match.group }}</span>
            <span class="px-2 py-0.5 rounded-full bg-slate-950 border border-slate-800 text-slate-400">{{ match.venue }}</span>
          </div>

          <!-- Teams and Scores -->
          <div class="flex items-center justify-between gap-4 py-2">
            <!-- Home Team -->
            <div class="flex-1 flex flex-col items-center sm:flex-row sm:items-center sm:space-x-4 text-center sm:text-left">
            ...
            <div class="flex items-center space-x-2">
              <input 
                type="number" 
                min="0"
                v-model="predictions[match.id].homeScore"
                placeholder="0"
              ...
              <input 
                type="number" 
                min="0"
                v-model="predictions[match.id].awayScore"
                placeholder="0"
                class="w-12 h-12 bg-slate-950 border border-slate-800 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 rounded-xl text-center font-bold text-lg text-white focus:outline-none transition-all duration-200"
              />
            </div>
            ...
            <!-- Away Team -->
            <div class="flex-1 flex flex-col items-center sm:flex-row-reverse sm:items-center sm:space-x-reverse sm:space-x-4 text-center sm:text-right">
