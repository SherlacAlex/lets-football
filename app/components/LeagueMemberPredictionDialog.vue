          ...
          <template v-else-if="detail">
            ...
            <div class="bg-slate-950/50 border border-slate-800 rounded-2xl p-4 space-y-3">
              <div class="flex items-center justify-between gap-2">
                ...
                <span v-if="detail.score_prediction" class="text-xs font-bold text-emerald-400">
            ...
            <div v-if="detail.answers.length" class="space-y-3">
              ...
              <div v-for="(answer, index) in detail.answers" :key="index"
                ...
                <p class="text-sm font-semibold text-white">{{ formatAnswerDisplay(answer) }}</p>
              </div>
            </div>
          </template>
        </div>
...
function formatAnswerDisplay(
  answer: MemberFixturePredictionDetail['answers'][number],
) {
