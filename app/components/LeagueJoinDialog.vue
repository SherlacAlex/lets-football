<template>
  <Teleport to="body">
    <div v-if="open" class="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6" role="dialog"
      aria-modal="true" aria-labelledby="join-league-title">
      <div class="absolute inset-0 bg-slate-950/80 backdrop-blur-sm" @click="close" />

      <div class="relative w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl p-6 space-y-5">
        ...
        <form class="space-y-4" @submit.prevent="submit">
          <div>
            <label for="invite-code" class="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">
              Invite code
            </label>
            <input id="invite-code" v-model="inviteCode" type="text" required maxlength="8" autocomplete="off"
